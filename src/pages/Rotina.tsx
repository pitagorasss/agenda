...
export function Rotina() {
  ...
  const activeUserId_ = activeUserId ?? authUser?.id ?? ''
  useEffect(() => {
    if (activeUserId_) {
  ...
  const activeUser = users.find((u) => u.id === activeUserId_)
  ...
  const isMyTab = authUser?.id === activeUserId_
  ...
  return (
    ...
    >
        ...
        <CardContent>
          <div className="grid grid-cols-7 gap-px rounded-xl border bg-muted overflow-hidden">
            ...
            {weekDays.map((d) => {
              ...
              const slots = slotsByWeekday.get(d.getDay()) ?? []
              const dayTasks = tasksByDate.get(dateKey) ?? []
              const hasRoutine = slots.length > 0
              const unmapped = dayTasks.filter((t) => !slotForTask(t, slots))
              return (
                <div
                  key={dateKey}
                  ...
                  className={cn(
                    'min-h-[140px] p-1.5 bg-background transition-colors hover:bg-accent/40',
                    isToday && 'ring-1 ring-brand-blue ring-inset',
                  )}
                >
                  {hasRoutine ? (
                    <div className="space-y-1.5">
                      {slots.map((s, i) => {
                        const done = completionKeys.has(`${s.id}|${dateKey}`)
                        const slotTasks = dayTasks.filter((t) => slotForTask(t, slots)?.id === s.id)
                        return (
                          <div
                            key={s.id}
                            ...
                            onDragOver={(e) => e.preventDefault()}
                            onDrop={(e) => handleDrop(e, dateKey, s.start_time)}
                          >
      ...
      {isMyTab && <RoutineConfigDialog open={configOpen} onOpenChange={setConfigOpen} />}
    </motion.div>
  )
}

function RoutineConfigDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (v: boolean) => void }) {
